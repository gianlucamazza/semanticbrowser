...
if grep -n "FROM.*as " Dockerfile Dockerfile.test 2>/dev/null; then
...
AS_COUNT=$(grep -c "FROM.*AS" Dockerfile Dockerfile.test 2>/dev/null || echo 0)
print_success "Found $AS_COUNT multi-stage build definitions"

# Verify Dockerfile syntax with buildx
if command -v docker &> /dev/null; then
    if docker buildx build --check -f Dockerfile . 2>&1 | grep -qi "FromAsCasing"; then
...
echo ""
print_info "Ready to build with: ./scripts/docker-build.sh"
print_info "Or start with: ./scripts/docker-up.sh"
